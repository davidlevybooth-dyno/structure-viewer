...
export function validateConfig(config: Partial<AppConfig>): string[] {
  ...
  if (config.api) {
    ...
    if (
      config.api.retries &&
      (config.api.retries < 0 || config.api.retries > 10)
    ) {
  ...
  if (config.sequence) {
    ...
    if (
      config.sequence.constraints?.maxSelections &&
      config.sequence.constraints.maxSelections < 1
    ) {
  ...
  if (config.performance) {
    ...
    if (
      config.performance.virtualization?.threshold &&
      config.performance.virtualization.threshold < 0
    ) {
