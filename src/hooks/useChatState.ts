...
export function useChatState({ 
  ...
  const pinned = useMemo(() => 
    filtered.filter((c) => c.pinned).sort((a, b) => (a.updatedAt < b.updatedAt ? 1 : -1)),
    [filtered]
  );
  ...
  const sendMessage = useCallback((convId: string, content: string) => {
    ...
    setTimeout(() => {
      const assistantMessage = createMessage("assistant", "Got it — I'll help with that.");
  ...
  const editMessage = useCallback((convId: string, messageId: string, newContent: string) => {
    ...
    setConversations((prev) =>
      prev.map((c) => {
        ...
        return {
          ...c,
          messages,
          preview: messages[messages.length - 1]?.content?.slice(0, 80) || c.preview,
        };
      })
    );
  }, []);

  const resendMessage = useCallback((convId: string, messageId: string) => {
