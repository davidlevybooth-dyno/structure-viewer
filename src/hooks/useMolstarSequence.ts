...
import { SequenceData, SequenceChain, SequenceResidue } from '@/components/sequence-interface/types';
  ...
  const extractSequenceData = useCallback(async () => {
    ...
    try {
      ...
      console.log('Plugin managers available:', {
        ...
        structureCount: plugin.managers?.structure?.hierarchy?.current?.structures?.length || 0
      });

      // Check if sequence manager is available
      const seqMgr = plugin.managers?.structure?.sequence;
      if (!seqMgr) {
        throw new Error('Sequence manager not available - plugin may not be fully initialized');
      }
      ...
      console.log('Found sequences from Molstar Sequence Manager:', sequences.length);
      ...
      console.log('Extracting sequences:', { pdbId, title, sequenceCount: sequences.length });
      ...
      for (const seqWrapper of sequences) {
        ...
        if (info.entityType === 'polymer' && seqWrapper.sequence) {
          ...
          if (residues.length > 0) {
            ...
            console.log(`Added chain ${chainId} with ${residues.length} residues:`, sequence.substring(0, 20) + '...');
          }
        }
      }
      ...
      const message = error instanceof Error ? error.message : 'Failed to extract sequence data';
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (plugin.managers?.structure?.hierarchy?.behaviors?.selection) {
        subscription = plugin.managers.structure.hierarchy.behaviors.selection.subscribe(() => {
