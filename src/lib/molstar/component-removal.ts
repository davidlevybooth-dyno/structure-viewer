...
export async function removeWaterMolecules(
  ...
  try {
    ...
    const waterSelection = Script.getStructureSelection(waterExpression, structure.cell.obj.data);
    ...
    if (allComponents.length > 0) {
      ...
      if (verbose) console.log(`✅ Removed water from ${allComponents.length} components`);
    }
  ...
  try {
    ...
    for (const ligandName of ligandNames) {
      const ligandSelection = MS.struct.generator.atomGroups({
        'residue-test': MS.core.rel.eq([MS.struct.atomProperty.macromolecular.label_comp_id(), ligandName])
      });
      
      const selection = Script.getStructureSelection(ligandSelection, structure.cell.obj.data);
      ...
      if (loci.elements?.length > 0) {
        ...
        await plugin.managers.structure.component.modifyByCurrentSelection(allComponents, 'subtract');
        totalRemoved = true;
        if (verbose) console.log(`✅ Removed ligand ${ligandName}`);
      }
    }
  ...
  try {
    ...
    for (const ionName of ionNames) {
      const ionSelection = MS.struct.generator.atomGroups({
        'residue-test': MS.core.rel.eq([MS.struct.atomProperty.macromolecular.label_comp_id(), ionName])
      });
      
      const selection = Script.getStructureSelection(ionSelection, structure.cell.obj.data);
      const loci = StructureSelection.toLociWithSourceUnits(selection);
      ...
      if (loci.elements?.length > 0) {
        ...
        await plugin.managers.structure.component.modifyByCurrentSelection(allComponents, 'subtract');
        totalRemoved = true;
        if (verbose) console.log(`✅ Removed ion ${ionName}`);
      }
    }
