...
export class selectionOperations {
  ...
  async highlightResidues(
    ...
    try {
      ...
      if (!hierarchy.structures.length) {
        return {
          success: false,
          error: this.createError("No structures loaded", "SELECTION_ERROR"),
        };
      }
      ...
      return {
        success: false,
        error: this.createError(
          `Failed to highlight residues: ${error}`,
          "SELECTION_ERROR",
          { ranges, options, originalError: error },
        ),
      };
    }
  }
  ...
  async hideResidueRange(
    ...
    return this.performResidueOperation(
      { chainId, start: startSeq, end: endSeq },
      "hide",
    );
  }
   ...
   */
  private async performResidueOperation(
    range: ResidueRange,
    ...
    try {
      ...
      return {
        success: false,
        error: this.createError(
          `Failed to perform ${operation} on residue range: ${error}`,
          "SELECTION_ERROR",
          { range, operation, originalError: error },
        ),
      };
    }
  }
   ...
   */
  private async hideResidueRangeInternal(
    range: ResidueRange,
    data: any,
    hierarchy: any,
  ): Promise<OperationResult> {
    try {
      ...
      return {
        success: false,
        error: this.createError(
          `Failed to hide residue range: ${error}`,
          "SELECTION_ERROR",
          { range, originalError: error },
        ),
      };
    }
  }
   ...
   */
  private async isolateResidueRangeInternal(
    range: ResidueRange,
    data: any,
    hierarchy: any,
  ): Promise<OperationResult> {
    try {
      ...
      return {
        success: false,
        error: this.createError(
          `Failed to isolate residue range: ${error}`,
          "SELECTION_ERROR",
          { range, originalError: error },
        ),
      };
